...
func UpdateSpec(world w.World, targetContainer *widget.Container, entity ecs.Entity) {
...

	{
		if entity.HasComponent(world.Components.Material) {
			...
			targetContainer.AddChild(common.NewBodyText(amount, colors.TextColor, world))
		}

		if entity.HasComponent(world.Components.Attack) {
			attack := world.Components.Attack.Get(entity).(*gc.Attack)
			targetContainer.AddChild(common.NewBodyText(attack.AttackCategory.String(), colors.TextColor, world))

			damage := fmt.Sprintf("%s %s", consts.DamageLabel, strconv.Itoa(attack.Damage))
			targetContainer.AddChild(common.NewBodyText(damage, colors.TextColor, world))

			accuracy := fmt.Sprintf("%s %s", consts.AccuracyLabel, strconv.Itoa(attack.Accuracy))
			targetContainer.AddChild(common.NewBodyText(accuracy, colors.TextColor, world))

			attackCount := fmt.Sprintf("%s %s", consts.AttackCountLabel, strconv.Itoa(attack.AttackCount))
			targetContainer.AddChild(common.NewBodyText(attackCount, colors.TextColor, world))
		...
		if entity.HasComponent(world.Components.Wearable) {
			...
			targetContainer.AddChild(common.NewBodyText(equipmentCategory, colors.TextColor, world))

			defense := fmt.Sprintf("%s %+d", consts.DefenseLabel, wearable.Defense)
			targetContainer.AddChild(common.NewBodyText(defense, colors.TextColor, world))
		...
		if entity.HasComponent(world.Components.Card) {
			...
			targetContainer.AddChild(common.NewBodyText(cost, colors.TextColor, world))
		}
	}
}
...
func damageAttrText(world w.World, dat gc.ElementType, str string) *widget.Text {
	...
	case gc.ElementTypeFire:
		text = common.NewBodyText(str, colors.FireColor, world)
	case gc.ElementTypeThunder:
		text = common.NewBodyText(str, colors.ThunderColor, world)
	case gc.ElementTypeChill:
		text = common.NewBodyText(str, colors.ChillColor, world)
	case gc.ElementTypePhoton:
		text = common.NewBodyText(str, colors.PhotonColor, world)
	default:
		text = common.NewBodyText(str, colors.TextColor, world)
	}
...
func addEquipBonus(targetContainer *widget.Container, equipBonus gc.EquipBonus, world w.World) {
	if equipBonus.Vitality != 0 {
		...
		targetContainer.AddChild(common.NewBodyText(vitality, colors.TextColor, world))
	}

	if equipBonus.Strength != 0 {
		strength := fmt.Sprintf("%s %+d", consts.StrengthLabel, equipBonus.Strength)
		targetContainer.AddChild(common.NewBodyText(strength, colors.TextColor, world))
	}

	if equipBonus.Sensation != 0 {
		sensation := fmt.Sprintf("%s %+d", consts.SensationLabel, equipBonus.Sensation)
		targetContainer.AddChild(common.NewBodyText(sensation, colors.TextColor, world))
	}

	if equipBonus.Dexterity != 0 {
		dexterity := fmt.Sprintf("%s %+d", consts.DexterityLabel, equipBonus.Dexterity)
		targetContainer.AddChild(common.NewBodyText(dexterity, colors.TextColor, world))
	}

	if equipBonus.Agility != 0 {
		agility := fmt.Sprintf("%s %+d", consts.AgilityLabel, equipBonus.Agility)
		targetContainer.AddChild(common.NewBodyText(agility, colors.TextColor, world))
	}
}
