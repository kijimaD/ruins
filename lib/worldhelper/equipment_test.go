...
func TestEquipDisarm(t *testing.T) {
	...
	item.AddComponent(world.Components.Game.Item, &gc.Item{})
	...
	// 装備されたことを確認
	assert.True(t, item.HasComponent(world.Components.Game.ItemLocationEquipped), "アイテムが装備されていない")
	assert.False(t, item.HasComponent(world.Components.Game.ItemLocationInBackpack), "アイテムがまだバックパックにある")
	assert.True(t, item.HasComponent(world.Components.Game.EquipmentChanged), "装備変更フラグが設定されていない")
	...
	// 装備が外されたことを確認
	assert.False(t, item.HasComponent(world.Components.Game.ItemLocationEquipped), "アイテムがまだ装備されている")
	assert.True(t, item.HasComponent(world.Components.Game.ItemLocationInBackpack), "アイテムがバックパックに戻っていない")
	assert.True(t, item.HasComponent(world.Components.Game.EquipmentChanged), "装備変更フラグが設定されていない")
