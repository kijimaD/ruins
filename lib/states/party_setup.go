...
func (st *PartySetupState) initUIWithFocus(world w.World, focusIndex int) *ebitenui.UI {
	...
	st.memberDescContainer = styled.NewVerticalContainer(
	...
	st.memberStatusContainer = styled.NewVerticalContainer(
		widget.ContainerOpts.BackgroundImage(world.Resources.UIResources.Panel.ImageTrans),
		widget.ContainerOpts.WidgetOpts(
	...
	// レイアウト構成
	st.rootContainer.AddChild(styled.NewTitleText("パーティ編成", world))
...
func (st *PartySetupState) updateMemberDisplay(world w.World) {
	...
	if st.selectedMemberEntity == nil {
		// 選択なしの場合
		st.memberDescContainer.AddChild(styled.NewBodyText("メンバーを選択してください", colors.TextColor, world))
		return
	}
	...
	if jobName != "" {
		...
		st.memberDescContainer.AddChild(styled.NewSubtitleText(fmt.Sprintf("%-8s %-8s", "職業:", jobName), world))
	} else {
		st.memberDescContainer.AddChild(styled.NewSubtitleText(fmt.Sprintf("%-8s %-8s", "職業:", "(なし)"), world))
	}

	if entity.HasComponent(world.Components.Pools) {
		pools := world.Components.Pools.Get(entity).(*gc.Pools)
		// 固定幅フォーマット: ラベル部分を8文字固定
		st.memberDescContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-8s %d", "レベル:", pools.Level), colors.TextColor, world))
		st.memberDescContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-8s %d", "経験値:", pools.XP), colors.TextColor, world))
	}
	...
	if entity.HasComponent(world.Components.Attributes) {
		...
		// 固定幅フォーマット: ラベル部分を8文字固定
		st.memberDescContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-8s %d", "攻撃力:", attrs.Strength.Total), colors.TextColor, world))
		st.memberDescContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-8s %d", "防御力:", attrs.Defense.Total), colors.TextColor, world))
	}

	// 装備情報
	st.memberDescContainer.AddChild(styled.NewBodyText("", colors.TextColor, world)) // 空行
	st.memberDescContainer.AddChild(styled.NewBodyText("装備:", colors.TextColor, world))
	...
	for i, slot := range wearSlots {
		if slot != nil {
			...
			st.memberDescContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-5s %s", fmt.Sprintf("防具%d:", i+1), itemName), colors.TextColor, world))
		} else {
			st.memberDescContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-5s %s", fmt.Sprintf("防具%d:", i+1), "(なし)"), colors.TextColor, world))
		}
	}
	...
	for i, slot := range cardSlots {
		if slot != nil {
			...
			st.memberDescContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-5s %s", fmt.Sprintf("手札%d:", i+1), itemName), colors.TextColor, world))
		} else {
			st.memberDescContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-5s %s", fmt.Sprintf("手札%d:", i+1), "(なし)"), colors.TextColor, world))
		}
	}
	...
	if entity.HasComponent(world.Components.Attributes) {
		...
		st.memberStatusContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-3s %3d(%+3d)", consts.VitalityLabel, attrs.Vitality.Total, attrs.Vitality.Modifier), colors.TextColor, world))
		st.memberStatusContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-3s %3d(%+3d)", consts.StrengthLabel, attrs.Strength.Total, attrs.Strength.Modifier), colors.TextColor, world))
		st.memberStatusContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-3s %3d(%+3d)", consts.SensationLabel, attrs.Sensation.Total, attrs.Sensation.Modifier), colors.TextColor, world))
		st.memberStatusContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-3s %3d(%+3d)", consts.DexterityLabel, attrs.Dexterity.Total, attrs.Dexterity.Modifier), colors.TextColor, world))
		st.memberStatusContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-3s %3d(%+3d)", consts.AgilityLabel, attrs.Agility.Total, attrs.Agility.Modifier), colors.TextColor, world))
		st.memberStatusContainer.AddChild(styled.NewBodyText(fmt.Sprintf("%-3s %3d(%+3d)", consts.DefenseLabel, attrs.Defense.Total, attrs.Defense.Modifier), colors.TextColor, world))
	}
}
