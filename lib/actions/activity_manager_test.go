...
func TestActivityManagerGetAllActiveActivities(t *testing.T) {
	...
	// アクティブなアクティビティは2つ（中断されたものは除く）
	if len(activeActivities) != 2 {
		t.Errorf("Expected 2 active activities, got %d", len(activeActivities))
	}

	// 正しいアクティビティがアクティブかチェック
	if _, exists := activeActivities[actor1]; !exists {
		t.Errorf("Expected actor1 to have active activity")
	}
	if _, exists := activeActivities[actor2]; exists {
		t.Errorf("Expected actor2 to not have active activity (paused)")
	}
	if _, exists := activeActivities[actor3]; !exists {
		t.Errorf("Expected actor3 to have active activity")
	}
}
...
func TestActivityManagerCleanup(t *testing.T) {
	...
	// アクティビティを開始
	activity1 := NewActivity(ActivityWait, actor1, 1) // すぐ完了
	...
	// 1つをキャンセル
	manager.CancelActivity(actor3, "テスト", world)

	// クリーンアップ実行前の確認
	summary := manager.GetActivitySummary()
	if summary["total"] != 1 { // 完了とキャンセルは自動削除されているはず
		t.Errorf("Expected 1 activity before cleanup, got %v", summary["total"])
	}

	// 手動クリーンアップ
	manager.CleanupCompletedActivities()

	// クリーンアップ後は実行中のアクティビティのみ残る
	summary = manager.GetActivitySummary()
	if summary["total"] != 1 {
		t.Errorf("Expected 1 activity after cleanup, got %v", summary["total"])
	}
